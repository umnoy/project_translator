    ...
    const std::vector<std::pair<std::string, std::string>>& headers)
{
    ...
    if (ec) {
        throw std::runtime_error("Ошибка DNS: " + ec.message());
    }
    ...
    if (ec) {
        throw std::runtime_error("Ошибка TCP подключения: " + ec.message());
    }

    stream.handshake(ssl::stream_base::client, ec);
    if (ec) {
        throw std::runtime_error("Ошибка SSL Handshake: " + ec.message());
    }
    ...
    if (ec) { 
        if(ec == net::error::eof) 
            throw std::runtime_error("Ошибка отправки: Сервер закрыл соединение преждевременно"); 
        else if (ec == ssl::error::stream_truncated)
            throw std::runtime_error("Ошибка отправки (SSL): Поток усечен во время записи");
        else
            throw std::runtime_error("Ошибка отправки: " + ec.message());
    }
    ...
    if (ec && ec != http::error::end_of_stream) {
        if (ec == ssl::error::stream_truncated)
            throw std::runtime_error("Ошибка чтения ответа (SSL): Поток усечен во время чтения");
        else
            throw std::runtime_error("Ошибка чтения ответа: " + ec.message());
    }

    if (res.result() != http::status::ok) { 
        std::string error_msg = "Сервер вернул статус ошибки: " + std::to_string(res.result_int());
        if (!res.body().empty()) {
            error_msg += ". Тело ответа: " + res.body();
        }
